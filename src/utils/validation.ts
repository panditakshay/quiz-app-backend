...
function validateAnswerSubmissionInput(
  userId: string,
  questionId: string,
  quizId: string,
...
function validateAndMarkQuestionAnswered(
  quizAttemptByUser: { answers: { questionId: string }[] },
  questionId: string,
) {
  if (
    quizAttemptByUser.answers.some((answer) => answer.questionId === questionId)
  ) {
    throw new Error("Question has already been answered.");
  }
}

function validateUserAttempt(
  quizAttemptsByUser: Map<
    string,
    Map<string, { answers: Answer[]; quizId: string }>
  >,
  userId: string,
) {
